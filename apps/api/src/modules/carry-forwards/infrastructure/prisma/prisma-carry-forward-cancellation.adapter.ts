...
import { assertCarryForwardCanBeCanceled } from '../../domain/carry-forward.policy';
import { mapCarryForwardRecordToItem } from '../mappers/carry-forward.mapper';
import { CarryForwardsService } from '../services/carry-forwards.service';

@Injectable()
export class PrismaCarryForwardCancellationAdapter implements CarryForwardCancellationPort {
  constructor(
    private readonly prisma: PrismaService,
    private readonly carryForwardsService: CarryForwardsService
  ) {}

  async cancelInWorkspace(
    input: Parameters<CarryForwardCancellationPort['cancelInWorkspace']>[0]
  ) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (!record) {
        throw notFoundError('취소할 차기 이월 기록을 찾을 수 없습니다.');
      }
      ...
      const [targetPeriod, targetOpeningBalanceSnapshot] = await Promise.all([
        ...
        tx.openingBalanceSnapshot.findUnique({
          where: { effectivePeriodId: record.toPeriodId }
        })
      ]);
      ...
      assertCarryForwardCanBeCanceled({
        ...
        targetUsageCounts: await readTargetUsageCounts(tx, {
          tenantId: input.tenantId,
          ledgerId: input.ledgerId,
      ...
      await tx.balanceSnapshotLine.deleteMany({
        where: { openingSnapshotId: targetOpeningBalanceSnapshot.id }
      });
    ...
    if (
      ...
      throw notFoundError('취소한 차기 이월 기록을 확인할 수 없습니다.');
    }
...
async function readTargetUsageCounts(
  ...
  const [
    ...
    tx.closingSnapshot.findUnique({
      where: { periodId: input.targetPeriodId },
      select: { id: true }
    })
  ]);
