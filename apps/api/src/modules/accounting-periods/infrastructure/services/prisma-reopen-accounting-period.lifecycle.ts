...
@Injectable()
export class PrismaReopenAccountingPeriodLifecycle {
  constructor(
  ...
  async execute(
    ...
    if (!period) {
      throw notFoundError('재오픈할 운영 기간을 찾을 수 없습니다.');
    }
    ...
    if (!reason) {
      throw validationError('재오픈 사유를 입력해 주세요.');
    }
    ...
    if (!refreshedPeriod) {
      throw notFoundError('재오픈 이후 운영 기간을 다시 불러오지 못했습니다.');
    }
