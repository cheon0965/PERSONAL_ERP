...
} from '../../domain/plan-item-generation.policy';
import {
  PlanItemGenerationPort,
  type LedgerTransactionFlowKindValue
} from '../ports/plan-item-generation.port';
import { PlanItemViewPort } from '../ports/plan-item-view.port';
...
export class GeneratePlanItemsUseCase {
  constructor(
    ...
    private readonly planItemViewPort: PlanItemViewPort
  ) {}
  ...
  async execute(
    ...
    if (!period) {
      throw notFoundError('계획 항목을 생성할 운영 기간을 찾을 수 없습니다.');
    }
    ...
    const defaultTypeIdByFlow = new Map<
      LedgerTransactionFlowKindValue,
      string
    >();
    ...
    const liabilityExpenseTypeId = defaultTypeIdByFlow.get('EXPENSE');
    ...
    for (const repaymentSchedule of liabilityRepaymentSchedules) {
      ...
      createData.push({
        ...
        title: [
          ...
          .filter(Boolean)
          .join(' '),
        plannedAmount: repaymentSchedule.totalAmount,
    ...
    const view = await this.planItemViewPort.findViewInWorkspace(
      workspace.tenantId,
      workspace.ledgerId,
      period.id
    );
    ...
    if (!view) {
      throw notFoundError('생성된 계획 항목 결과를 다시 불러오지 못했습니다.');
    }
...
function resolveAutoCollectedTransactionStatus(input: {
  ...
  if (
    input.flowKind === 'TRANSFER' ||
    input.flowKind === 'OPENING_BALANCE' ||
    input.flowKind === 'CARRY_FORWARD'
  ) {
