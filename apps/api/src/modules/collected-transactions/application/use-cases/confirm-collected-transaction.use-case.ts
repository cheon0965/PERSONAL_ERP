...
import { readParsedImportedRowPayload } from '../../../import-batches/public';
import {
  assertBalancedJournalAdjustmentLines,
  assertJournalEntryCanBeReversed,
  buildReversalJournalLines
} from '../../../journal-entries/public';
import { assertCollectedTransactionCanBeCorrected } from '../../domain/collected-transaction-transition.policy';
import {
  ConfirmCollectedTransactionStorePort,
  type ConfirmationCollectedTransaction,
  type ConfirmTransactionContext
} from '../ports/confirm-collected-transaction-store.port';
import { REQUIRED_CONFIRM_ACCOUNT_SUBJECT_CODES } from '../policies/confirm-collected-transaction.policy';
import {
  assertConfirmationAllowed,
  assertConfirmationTransactionFound,
  assertConfirmationTransactionHasPeriod
} from '../policies/confirm-collected-transaction.validator';
import {
  buildConfirmationEntryNumber,
  buildConfirmationJournalLines,
  resolveConfirmationAccountSubjectIds
} from '../policies/confirm-collected-transaction.factory';
    ...
    const journalEntry = await this.confirmStore.runInTransaction(
      async (ctx) => {
        ...
        if (
          ...
          throw conflictError('부채 상환 스케줄 상태 갱신에 실패했습니다.');
        }
  ...
  private async confirmImportedReversalTransaction(input: {
    ...
    if (reversedJournalEntryCount !== 1) {
      ...
      if (!currentJournalEntryStatus) {
        throw notFoundError('Original journal entry not found.');
      }
    ...
    if (correctedCollectedTransactionCount !== 1) {
      ...
      if (!currentCollectedTransactionStatus) {
        throw notFoundError('Original collected transaction not found.');
      }
