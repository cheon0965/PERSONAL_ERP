...
} from '../../application/policies/bulk-collect-imported-rows.policy';
...
import { IMPORT_COLLECTION_LOCK_TTL_MS } from '../../domain/import-batch-collection-job.constants';
...
@Injectable()
export class BulkCollectImportedRowsHandler {
  constructor(
  ...
  async execute(
    ...
    if (!normalizedRequest.fundingAccountId) {
      throw validationError('일괄 등록에 사용할 계좌/카드를 선택해 주세요.');
    }
    ...
    if (!batch) {
      throw notFoundError('업로드 배치를 찾을 수 없습니다.');
    }
    ...
    if (requestedRowIds && rows.length !== requestedRowIds.length) {
      throw notFoundError('선택한 업로드 행 일부를 찾을 수 없습니다.');
    }
