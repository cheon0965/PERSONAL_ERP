...
import { FinancialStatementGenerationPort } from '../ports/financial-statement-generation.port';
import { FinancialStatementViewPort } from '../ports/financial-statement-view.port';
import { buildStatementPayloads } from '../projectors/financial-statement-payload.projector';
...
export class GenerateFinancialStatementsUseCase {
  constructor(
    ...
    private readonly financialStatementViewPort: FinancialStatementViewPort
  ) {}
  ...
  async execute(
    ...
    if (!period) {
      throw notFoundError('재무제표를 생성할 운영 기간을 찾을 수 없습니다.');
    }
    ...
    const view = await this.financialStatementViewPort.findViewInWorkspace(
      workspace.tenantId,
      workspace.ledgerId,
      period.id
    );
    ...
    if (!view) {
      throw notFoundError('생성된 재무제표 결과를 다시 불러오지 못했습니다.');
    }
