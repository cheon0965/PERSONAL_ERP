...
const boundaryTestFile = path.join(
  repoRoot,
  'apps/api/test/architecture/legacy-transaction-boundary.test.ts'
);
...
test('phase1 backbone import surface stays confined to explicit seed, backfill, and integration bootstrap files', () => {
  const allowedFiles = [
    ...
    'apps/api/test/integration/prisma/collected-transactions.prisma.integration.test.ts',
    'apps/api/test/support/prisma/context.ts'
  ].sort();
  const files = [
    ...listTypeScriptFiles(path.join(repoRoot, 'apps/api/prisma')),
