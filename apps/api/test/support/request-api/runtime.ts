...
import { createPrismaMock } from './prisma-mock/index';
import { createRequestTestState } from './state/index';
import type { RequestTestContext, RequestTestState } from './types';
...
export async function createRequestTestContext(): Promise<RequestTestContext> {
  ...
  try {
    ...
    // 요청 배선 검증을 빠르게 유지하기 위해 Prisma와 JWT는 인메모리 픽스처 저장소로 대체한다.
