...
test(
  'Real API/DB integration covers accounting period open -> import batch -> collect -> confirm -> close',
  async (t) => {
    let context: RealApiPrismaIntegrationContext | null = null;
    let fixture:
      | Awaited<ReturnType<typeof createIntegrationWorkspaceFixture>>
      | null = null;
...
test(
  'Real API/DB integration covers recurring rule -> plan items -> auto-matched import collect -> confirm -> financial statements',
  async (t) => {
    let context: RealApiPrismaIntegrationContext | null = null;
    let fixture:
      | Awaited<ReturnType<typeof createIntegrationWorkspaceFixture>>
    ...
    try {
      ...
      assert.equal(
        positionStatement?.payload.summary[0]?.amountWon,
        1_125_000
      );
      assert.equal(
