...
import { canEditCollectedTransaction } from '@/features/transactions/model/transaction-workflow';
import {
  buildErrorFeedback,
...
import { buildVehicleOperatingSummaryView } from '../model/vehicles.summary';
...
type SaveVehicleMaintenanceMutationInput = {
  ...
  payload: CreateVehicleMaintenanceLogRequest;
...
export function VehicleMaintenanceForm({
  ...
  const linkedCollectedTransaction =
    initialMaintenanceLog?.linkedCollectedTransaction;
  ...
  const includeInactiveFundingAccounts =
    mode === 'edit' &&
    Boolean(
      initialMaintenanceLog?.linkedCollectedTransaction?.fundingAccountId
    );
  const includeInactiveCategories =
    mode === 'edit' &&
    Boolean(initialMaintenanceLog?.linkedCollectedTransaction?.categoryId);
  ...
  React.useEffect(() => {
    ...
    form.setValue(
      'categoryId',
      selectedVehicle?.defaultMaintenanceCategoryId ?? ''
    );
      ...
      onSubmit={form.handleSubmit(async (values) => {
        ...
        if (isAccountingLocked) {
          setFeedback({
            severity: 'error',
            message:
              '이미 확정된 회계 연동이 있어 정비 기록을 수정할 수 없습니다. 연결 전표 상세에서 반전 또는 정정으로 조정해 주세요.'
          });
          return;
        }
        ...
        if (values.accountingEnabled) {
          ...
          if (values.amountWon <= 0) {
            setFeedback({
              severity: 'error',
              message: '회계 연동을 켠 정비 기록은 금액이 0보다 커야 합니다.'
            });
            return;
          }
        }
        ...
        try {
          ...
          notifySuccess(
            mode === 'edit'
              ? '정비 기록을 수정했고 회계 연동 상태까지 함께 반영했습니다.'
              : '정비 기록을 저장했고 회계 연동 상태까지 함께 반영했습니다.'
          );
        } catch (error) {
          setFeedback(
            buildErrorFeedback(
    ...
    >
      <Stack spacing={appLayout.cardGap}>
        <Alert severity="info" variant="outlined">
          정비 이력은 차량 운영 판단용 기록입니다. 회계 연동을 켜면 이 저장과
...
function buildDefaultValues(
  ...
  return {
    ...
    categoryId: selectedVehicle?.defaultMaintenanceCategoryId ?? ''
  };
}
...
function mapMaintenanceLogToFormInput(
  ...
  return {
    ...
    accountingEnabled: Boolean(maintenanceLog.linkedCollectedTransaction),
    fundingAccountId:
      maintenanceLog.linkedCollectedTransaction?.fundingAccountId ?? '',
    categoryId: maintenanceLog.linkedCollectedTransaction?.categoryId ?? ''
  };
}
