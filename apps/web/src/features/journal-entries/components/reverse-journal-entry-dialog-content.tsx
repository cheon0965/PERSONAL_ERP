...
import { findAccountingPeriodForDate } from '@/features/accounting-periods/model/accounting-period-selection';
...
import {
  ...
  reverseJournalEntry
} from '../api/journal-entries.api';
import {
  buildReverseDefaultValues,
  invalidateAdjustmentQueries,
  trimOptionalText
} from '../model/journal-entry-adjustment-dialog.shared';
import type {
  JournalEntryAdjustmentContentProps,
  ReverseJournalEntryFormInput,
  SubmitFeedback
} from '../model/journal-entry-adjustment-dialog.types';
import { reverseJournalEntrySchema } from '../model/journal-entry-adjustment-dialog.types';
      ...
      onSubmit={form.handleSubmit(async (values) => {
        ...
        if (
          ...
          setFeedback({
            severity: 'error',
            message:
              '반전 전표 일자는 최신 전표 입력 가능월 안에 있어야 합니다.'
          });
          return;
        }
        ...
        <TextField
          ...
          helperText={
            ...
            (journalWritablePeriods.length > 1
              ? '최신 전표 입력 가능월 안에서만 반전할 수 있습니다.'
              : adjustmentPeriod
                ? `${adjustmentPeriod.monthLabel} 최신 전표 입력 가능월 안에서만 반전할 수 있습니다.`
