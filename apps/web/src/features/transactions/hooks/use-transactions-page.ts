...
} from '@/features/accounting-periods/model/accounting-period-selection';
import {
  getJournalEntries,
  journalEntriesQueryKey
} from '@/features/journal-entries/api/journal-entries.api';
import {
...
} from '../pages/transactions-page.commands';
import {
  buildCategoryOptions,
  buildFundingAccountOptions,
  filterTransactions,
  prioritizeVisibleTransactions
} from '../pages/transactions-page.filters';
import { buildJournalEntryFallbackItem } from '../pages/transactions-page.shared';
import {
...
import { canConfirmCollectedTransaction } from '../model/transaction-workflow';
...
export function useTransactionsPage() {
  ...
  useDomainHelp({
    ...
    primaryEntity: '수집 거래',
    relatedEntities: ['운영 월', '거래 유형', '자금수단', '거래 분류', '전표'],
    truthSource:
    ...
    supplementarySections: [
      {
        ...
        items: [
          ...
          '전표 준비 상태가 되면 행의 전표 확정을 누르거나 여러 행을 선택해 선택 전표 확정으로 한 번에 전표를 생성합니다.'
        ]
      },
      {
        title: '목록 조작 팁',
