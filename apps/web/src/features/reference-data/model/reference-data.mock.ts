...
function readUserManagedManagementNote(input: {
  ...
  return input.currentRoleOwnsPreparation
    ? '사용자 관리 데이터이지만, 앱에서 직접 생성하거나 수정하는 화면은 제공하지 않습니다. 준비 상태를 확인하고 운영 준비 절차에 따라 추가/정비 범위를 진행해야 합니다.'
    : '사용자 관리 데이터이며, 소유자 또는 관리자가 준비 상태를 책임집니다. 앱에서 직접 생성하거나 수정하는 화면은 제공하지 않습니다.';
}
