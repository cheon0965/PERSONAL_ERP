...
- `npm run check:quick`는 Prettier, 문서의 `npm run` 명령 정합성 검사, 문서의 Web/API 표면 정합성 검사, 금액 raw 연산 가드, lint, typecheck를 함께 확인합니다.
...
- `npm run docs:check:npm-run`는 `README.md`, `CONTRIBUTING.md`, `ENVIRONMENT_SETUP.md`, `docs/**/*.md`의 `npm run` 표기를 루트/워크스페이스 스크립트와 대조합니다.
- `npm run docs:check:surface`는 `docs/API.md`, `docs/CURRENT_CAPABILITIES.md`, `docs/OPERATIONS_CHECKLIST.md`, `docs/VALIDATION_NOTES.md`의 Web/API 표면이 실제 `apps/web/app` 라우트와 controller 기반 Swagger 표면과 맞는지 확인합니다.
...
- `npm run test:e2e:smoke:build:browser`는 CI와 동일한 루트 래퍼 명령이며, 내부적으로 Web 워크스페이스의 브라우저 빌드 스모크를 호출합니다.
...
- `npm run docs:check`: 통과
  ...
  - Web 라우트 61개, API operation 136개, `docs/API.md` 라우트 맵 61개 확인
  - `docs/CURRENT_CAPABILITIES.md`, `docs/OPERATIONS_CHECKLIST.md`, `docs/VALIDATION_NOTES.md`의 Web/API 표면 정합성 확인
- 수동 대조:
  - `docs/` 전체 Markdown 제목 구조와 완료 문서 인덱스를 확인
  - `apps/web/app` 라우트, `apps/api/src/modules/*Controller`, `packages/contracts/src` 계약 파일을 현재 구현 표면 기준으로 대조
...
- `npm run docs:check`: 통과
- `npm run test:security:api`: 통과, 테스트 315, pass 309, skipped 6
- `npm run test:web`: 통과, 테스트 31, pass 31
...
- `validate`
  `npm run check:quick`와 `npm run test`를 조합해 포맷, 문서 명령 정합성, 문서 Web/API 표면 정합성, lint, typecheck, 기본 테스트를 확인합니다.
- `e2e-smoke`
  `npm run test:e2e:smoke:build:browser`로 CI와 같은 production 빌드/start 기준 브라우저 스모크를 다시 확인합니다.

- `security-regression`
  `npm run test:security:api`로 인증/세션, 브라우저/API 경계 회귀를 CI에서 다시 확인합니다.
- `prisma-integration`
  `npm run test:prisma`로 disposable MySQL을 부팅하고 마이그레이션과 실제 MySQL 경계 시나리오를 수행합니다. 더 이상 GitHub secret 부재를 성공 skip으로 처리하지 않습니다.
- `audit-runtime`
  `npm run audit:runtime`으로 실제 배포 대상인 `api`, `web` 워크스페이스의 런타임 의존성을 점검하고, 현재 CI 게이트는 `high` 임계값 기준으로 실패를 판정합니다. 예외가 필요하면 `security/runtime-audit-allowlist.json`에 만료일과 사유를 남기며, 만료되었거나 더 이상 필요 없는 entry는 gate가 바로 실패합니다.
- `semgrep-ce`
...
- 로컬 기본 검증은 `check:quick`와 테스트가 맡고, CI는 여기에 품질/보안 게이트를 추가해 회귀를 막습니다.
...
- `semgrep-ce`는 빌드 산출물, 테스트 산출물, 마이그레이션, 운영 보조 script만 제외하고 애플리케이션 코드는 스캔 대상으로 둡니다.
...
- 회원가입 `POST /auth/register`, 이메일 인증 `POST /auth/verify-email`, 인증 메일 재발송 `POST /auth/resend-verification`, 초대 수락 `POST /auth/accept-invitation`
  회원가입 성공 시 refresh cookie를 발급하지 않는지, 필수 약관/개인정보 동의 누락 차단, 인증 전 로그인 차단, 인증 완료 후 워크스페이스/원장/OWNER membership 초기화, 중복 이메일 일반화 응답, 잘못된/만료/소비된 토큰, 재발송 토큰 교체, 초대 수락 후 membership 활성화, register/verify/resend rate limit, allowlist 밖 origin 차단을 검증
- 인증 세션 생성/회전/로그아웃
- 보호 라우트의 `401`
- `GET /auth/me`
...
- `GET /settings/workspace`, `PATCH /settings/workspace`
  현재 워크스페이스/원장 설정 조회와 Owner/Manager 수정, Viewer 거부를 검증
- 관리자 `GET /admin/members`, `POST /admin/members/invitations`, `PATCH /admin/members/:membershipId/role`, `GET /admin/audit-events`
  현재 워크스페이스 멤버 목록, 초대 메일 발송, 역할 변경, 마지막 활성 Owner 보호, 권한 거부 감사 이벤트, 감사 로그 조회를 검증
- 관리자 `GET /admin/policy`
  현재 워크스페이스의 Owner/Manager가 권한 정책 요약을 조회할 수 있는지 검증
...
- 전체 관리자 `GET /admin/support-context`, `POST /admin/support-context`, `DELETE /admin/support-context`
  지원 문맥 설정/해제, 세션 기준 현재 워크스페이스 반영, 일반 사용자 접근 차단을 검증
...
- `GET /funding-accounts`, `GET /categories`, `GET /account-subjects`, `GET /ledger-transaction-types`, `GET /insurance-policies`, `GET /liabilities`, `GET /liabilities/overview`, `GET /vehicles`, `GET /vehicles/operating-summary`
  현재 워크스페이스/원장 기준 활성 참조 데이터와 운영 보조 자산 데이터만 반환하는지 검증
- `GET /insurance-policies?includeInactive=true`, `POST /insurance-policies`, `PATCH /insurance-policies/:id`
  Owner/Manager 전용 보험 계약 생성, 수정, 비활성화/재활성화와 워크스페이스 범위 접근통제를 검증
...
- `POST /vehicles`, `PATCH /vehicles/:id`
  Owner/Manager 전용 차량 기본 정보 생성, 수정과 워크스페이스 범위 접근통제를 검증
- `GET /vehicles/fuel-logs`, `POST /vehicles/:id/fuel-logs`, `PATCH /vehicles/:vehicleId/fuel-logs/:fuelLogId`
  Owner/Manager 전용 차량 연료 이력 생성, 수정, 선택적 수집거래 생성, 차량 연결 수집거래의 직접 수정 차단, 전표 확정 후 차량 기록 수정 차단, 워크스페이스 범위 접근통제를 검증
- `GET /vehicles/maintenance-logs`, `POST /vehicles/:id/maintenance-logs`, `PATCH /vehicles/:vehicleId/maintenance-logs/:maintenanceLogId`
  Owner/Manager 전용 차량 정비 이력 생성, 수정, 선택적 수집거래 생성, 워크스페이스 범위 접근통제를 검증
- `GET /reference-data/readiness`
  현재 워크스페이스 기준 기준 데이터 readiness, ownership 구분, 부족 항목 요약을 검증
- `POST /funding-accounts`, `POST /funding-accounts/:id/bootstrap`, `PATCH /funding-accounts/:id`, `DELETE /funding-accounts/:id`
  Owner/Manager 전용 자금수단 생성, 신규 은행/카드 `bootstrapStatus=PENDING`, 기초금액 입력 기반 기초전표 발행과 초기화 완료, 금액 없는 완료 전환, 이름 변경, 비활성화/재활성화, 비활성 자금수단 종료, 미사용 자금수단 삭제, 거래내역이 있는 자금수단 삭제 차단과 워크스페이스 범위 접근통제를 검증
- `POST /categories`, `PATCH /categories/:id`
  Owner/Manager 전용 카테고리 생성, 이름 변경, 비활성화/재활성화와 워크스페이스 범위 접근통제를 검증
...
- `GET /collected-transactions`, `POST /collected-transactions`, `POST /collected-transactions/:id/confirm`
  DTO validation, 현재 워크스페이스 접근 범위 내 참조 검증, 생성/확정 응답 shape와 전표 연계를 검증
...
- `POST /recurring-rules`
  DTO validation, 현재 워크스페이스 접근 범위 내 계정/카테고리 검증, 생성 응답 shape
...
- `POST /import-batches/:id/rows/:rowId/collect`
  최신 진행월 범위 수집, 운영 중 임의 과거월/신규 운영월 생성 차단, 신규 계좌/카드 초기화 자동 운영월 생성과 완료 전환, 신규 계좌/카드 초기화의 회계 이력 가드, 반복 수집 거래 흡수 claim, 이미 다른 업로드 행과 연결된 대상의 `409 Conflict`, 같은 업로드 행 재수집 방지를 검증
- `POST /import-batches/:id/rows/collect`, `GET /import-batches/:id/collection-jobs/active`, `GET /import-batches/:id/collection-jobs/:jobId`, `POST /import-batches/:id/collection-jobs/:jobId/cancel`
  선택 행 또는 등록 가능 행 전체의 일괄 등록 Job 생성, 입출금 방향 기반 수입/지출/취소 유형 추론, 거래유형별 카테고리/메모 적용, 운영월 자동 생성 사유별 결과 메시지, 진행률/행별 결과 조회, 일괄 등록 Job 중단 요청, 같은 워크스페이스 내 동시 일괄 등록 잠금과 단건 등록 충돌 보호를 검증
...
- `OperationalAuditPublisher`
  외부 감사 sink 실패가 워크스페이스 감사 이벤트/기간 상태 이력/업로드 Job/전표 조정 발행 경계에서 핵심 회계 트랜잭션을 깨지 않도록 비동기 실패 흡수를 검증
- 대시보드 요약 계산
- 예측 잔액 계산
...
- `test:prisma`
  실제 MySQL 기준으로 현재 워크스페이스 접근 범위 확인, collected transaction repository boundary, 실제 API를 통한 `register -> verify-email -> login -> auth/me`, 기간/업로드/수집/전표/보고/재오픈/차기 이월 보호 대표 시나리오를 함께 검증
- `GET /collected-transactions`
  현재 구현 기준 current 워크스페이스 범위만 반환하는지, 내부 접근 제어 필드를 노출하지 않는지 검증
- `GET /recurring-rules`
  현재 구현 기준 current 워크스페이스 범위만 반환하는지, 내부 접근 제어 필드를 노출하지 않는지 검증
- `GET /dashboard/summary`
  다른 워크스페이스/원장 데이터가 집계에 섞이지 않고 raw read model을 노출하지 않는지 검증
- `GET /funding-account-status/summary`
  현재 구현 기준 선택 기간/자금수단 범위에서 수집 거래 기준과 확정 전표 기준의 자금수단별 수입, 지출, 이체, 잔액 현황을 반환하는 read model입니다. 전용 요청 단위 회귀 테스트로 기간과 자금수단 필터, current 워크스페이스 범위, 거래 목록, 경고 메시지, 자금수단별 합계가 함께 검증됩니다.
- `GET /forecast/monthly`
  현재 구현 기준 현재 워크스페이스 집계만 사용하고 month query를 그대로 반영하는지 검증
- `GET /operations/summary`, `GET /operations/checklist`, `GET /operations/exceptions`, `GET /operations/month-end`, `GET /operations/import-status`, `GET /operations/system-status`, `GET /operations/alerts`, `GET/POST /operations/exports`, `GET/POST /operations/notes`
  운영 허브 read model, checklist/예외/마감/업로드 현황, 시스템 상태/알림, 수동 UTF-8 CSV 반출, 운영 메모 저장과 감사 로그 기록을 검증

### Web

- env 파싱
- 데모 대체 응답 활성화/비활성화 정책
- 보호 요청의 Bearer 토큰 주입
- `401` 응답 시 세션 정리 정책
- mutation 요청의 JSON body 직렬화
- 회원가입/이메일 인증/인증 메일 재발송 auth API helper의 요청 path와 body 직렬화
- `/accept-invitation`, `/admin`, `/admin/users`, `/admin/tenants`, `/admin/support-context`, `/admin/security-threats`, `/admin/operations`, `/admin/members`, `/admin/logs`, `/admin/policy` 관리자/초대/권한 정책 라우트와 admin API helper의 보호 요청 path, Bearer token, mutation body 직렬화
- `/settings/workspace`, `/settings/account/profile`, `/settings/account/password`, `/settings/account/sessions`, `/settings/account/events` 설정 라우트와 워크스페이스/account API helper의 보호 요청 path, Bearer token, mutation body 직렬화
- `/operations`, `/operations/checklist`, `/operations/exceptions`, `/operations/month-end`, `/operations/imports`, `/operations/status`, `/operations/alerts`, `/operations/exports`, `/operations/notes` 운영 지원 라우트와 operations API helper의 보호 요청 path, Bearer token, mutation body 직렬화
- 요청 실패 메시지 안내
...
- `npm run test:e2e:smoke:build:browser`로 로그인/세션 복원, 운영 체크리스트 핵심 CTA, 현재 작업 기준 대체 응답 같은 브라우저 빌드 스모크를 루트 래퍼 경로로 검증
- `npm run test:e2e:smoke:build`는 브라우저 없는 in-process production 빌드/start 경로와 health 라우트 기준 최소 HTTP 스모크가 필요할 때 별도로 검증
- CI의 `e2e-smoke` job은 개발 서버가 아니라 빌드 결과물 기준 브라우저 스모크를 실행
- 실제 브라우저 상호작용으로 `dashboard`, `funding-account-status`, `reference-data`, `periods`, `insurances`, `vehicles`, `recurring`, `plan-items`, `imports`, `transactions`, `journal-entries`, `financial-statements`, `carry-forwards`, `forecast`, `settings`, `admin`, `operations`의 대표 운영 체크리스트 empty state, readiness 경고, 대체 응답 CTA가 유지되는지 검증
- 기준 데이터 CRUD, 반복 규칙 CRUD, 보험 계약 CRUD, 차량 기본 정보 CRUD 브라우저 검증은 현재 `npm run test:e2e` 전체 브라우저 회귀 범위에 남기고, CI 스모크에서는 제외합니다.
...
- GitHub `prisma-integration` 워크플로도 disposable MySQL 기반 경로로 통과 확인을 완료했습니다.
- 따라서 CI required check 연결과 Prisma 통합 검증 첫 통과는 더 이상 미완료 공백으로 보지 않고, CI 구성 변경 시 재확인하는 유지보수 항목으로 관리합니다.

## 2026-04-05 런타임 Audit 후속 조치 이력

- 실행: `npm run audit:runtime:full`과 동등한 런타임 `npm audit --omit=dev --workspace @personal-erp/api --workspace @personal-erp/web --json`
...
- 같은 날 npm registry 기준 최신 배포 버전도 `@nestjs/config 4.0.3`, `@nestjs/swagger 11.2.6`이며, 패키지 내부 의존성이 각각 `lodash 4.17.23`, `path-to-regexp 8.3.0`으로 고정돼 있어 로컬 비파괴 업그레이드 경로가 확인되지 않았습니다.
- 결론: 당시에는 CI gate를 `critical` 기준으로 유지하고, 위 4건은 upstream 릴리스 또는 안전한 대체 경로가 나올 때까지 `tracked exception`으로 관리했습니다.

## 2026-04-22 런타임 Audit 후속 조치

- 실행: `npm run audit:runtime`과 런타임 `npm audit --omit=dev --workspace @personal-erp/api --workspace @personal-erp/web --json`
- 조치:
  `@nestjs/config 4.0.3 -> 4.0.4`
  `@nestjs/swagger 11.2.6 -> 11.3.2`
  `next 15.5.14 -> 15.5.15`
- 결과:
  `lodash 4.18.1`, `path-to-regexp 8.4.2`, `next 15.5.15`로 갱신되었고 런타임 audit 결과는 `0 vulnerabilities`입니다.
- 결론: 2026-04-05 기준 tracked exception은 해소 완료로 전환했습니다.

## 2026-04-22 런타임 Audit Gate 강화
...
- 결과:
  당시 런타임 audit full 결과는 `0 vulnerabilities`였고, allowlist entry도 `0`건입니다. 이후의 현재 판정은 위 최신 날짜별 보완사항 기록을 우선합니다.
- 결론:
