...
- ASVS를 `checklist 전체 암기`가 아니라 `보안 설계와 검증의 기준선`으로 사용합니다.
- 모든 요구사항을 한 번에 구현하지 않습니다.
- 실제 기능이 아직 없는 영역은 `N/A` 또는 `deferred`로 명확히 남깁니다.
- 구조 과시용 보안 기능은 넣지 않습니다.
- 각 단계는 반드시 `코드 + test + 문서 + 운영 증거` 중 둘 이상을 남깁니다.
...
- 핵심 쓰기 흐름에는 워크스페이스 기준 접근통제가 들어가 있습니다.
...
- 회원가입, 비밀번호 변경, 비밀번호 재설정은 common/컨텍스트-derived password를 차단합니다.
...
| P2. 브라우저/API 경계         | CORS allowlist, 보안 헤더, `no-store`, origin 차단 적용                       | API 초기화, Web security header 테스트       |
| P3. CI 보안 게이트            | security regression, 런타임 audit, Semgrep, Gitleaks, Prisma integration 정리 | `.github/workflows/ci.yml`, `package.json`   |
| P4. 보안 이벤트/운영 로그     | 로그인/refresh/origin/권한/readiness 실패 이벤트를 request-id와 연결          | 운영 로그, `ERROR_HANDLING_AND_LOGGING.md`   |
| P5. 접근통제/데이터 보호 증적 | current 워크스페이스 범위와 최소 응답 shape를 요청 테스트로 고정              | `docs/API.md`, `VALIDATION_NOTES.md`         |
...
런타임 의존성 audit의 날짜별 이력과 advisory 판정은 `VALIDATION_NOTES.md`를 우선합니다.
...
- current 워크스페이스 범위 바깥 데이터가 목록/집계 응답에 섞이지 않는다는 증적이 있다.
