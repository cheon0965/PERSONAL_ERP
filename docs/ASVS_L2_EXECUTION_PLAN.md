...
- CI에는 `validate`, `e2e-smoke`, `security-regression`, `audit-runtime`, `semgrep-ce`, `gitleaks`가 들어갔지만, GitHub 첫 통과 증적과 required check 연결은 운영 단계에서 다시 확인해야 합니다.
...
| 보안 검증/SDLC     | `CI` 워크플로에 validate + e2e-smoke + security-regression + audit-runtime + Semgrep CE + Gitleaks 존재 | `부분 충족`   |
...
- `.github/workflows/ci.yml`에 `validate`, `e2e-smoke`, `security-regression`, `audit-runtime`, `semgrep-ce`, `gitleaks` job을 정리했다.
