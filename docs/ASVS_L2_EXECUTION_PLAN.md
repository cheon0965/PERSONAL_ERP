...
- 이 프로젝트는 개인 재무/현금흐름 데이터를 다루므로 `Level 2` 목표가 과하지 않습니다.
...
- 핵심 쓰기 흐름에는 사용자 소유권 검증이 들어가 있습니다.
...
- 로그인 성공/실패, refresh 실패/재사용, origin 차단, bearer 거부, ownership 거부, readiness 실패가 보안 이벤트로 남습니다.
...
목적:
현재 ownership 검증을 `실제로 동작하는 보호 규칙`이자 `포트폴리오와 ASVS L2에서 설명 가능한 증적`으로 고정합니다.
...
- 사용자 범위 바깥 데이터가 목록/집계 응답에 섞이지 않는다는 증적이 있다.
