...
- CI에는 `validate`, `e2e-smoke`, `security-regression`, `prisma-integration`, `audit-runtime`, `semgrep-ce`, `gitleaks`가 들어갔지만, GitHub 첫 통과 증적과 required check 연결은 운영 단계에서 다시 확인해야 합니다.
...
- 현재 `audit-runtime` gate는 `critical` 임계값 기준 실패를 사용하므로, 남아 있는 `high` advisory는 별도 대응 또는 예외 판단이 필요하다.
...
- 남은 4건은 모두 최신 `@nestjs/config 4.0.3`, `@nestjs/swagger 11.2.6`이 내부에서 `lodash 4.17.23`, `path-to-regexp 8.3.0`을 exact dependency로 유지하는 데서 왔습니다.
- 따라서 이번 라운드의 판정은 “즉시 로컬 업그레이드 불가, tracked exception으로 문서화”입니다.
