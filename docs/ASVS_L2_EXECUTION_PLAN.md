...
| 보안 검증/SDLC     | `CI` 워크플로에 validate + e2e-smoke + security-regression + prisma-integration + audit-runtime + Semgrep CE + Gitleaks 존재 | `부분 충족`   |
...
- `.github/workflows/ci.yml`에 `validate`, `e2e-smoke`, `security-regression`, `prisma-integration`, `audit-runtime`, `semgrep-ce`, `gitleaks` job을 정리했다.
...
- 현재 `audit-runtime` gate는 `high` 임계값 기준 실패를 사용하며, 예외는 `security/runtime-audit-allowlist.json`에서 만료일과 사유가 있는 entry만 허용한다.
...
- 기본 검증, build 결과물 smoke, 보안 회귀 테스트, runtime audit, SAST, secret scan이 각각 분리된 증적으로 남는다.
- 로컬 검증 기준과 CI 보안 게이트 기준은 `docs/VALIDATION_NOTES.md`와 PR 템플릿에 반영했다.
- 2026-04-22 기준 GitHub CI 첫 통과 증적과 required check 연결, Docker 환경 `npm run test:prisma`, GitHub `prisma-integration` 통과 확인까지 완료된 상태로 반영했다.
...
1. 운영 HTTPS + `Strict-Transport-Security` + `SWAGGER_ENABLED` 리허설: 남음
2. CI 보안 job 첫 통과 증적과 required check 연결 확인: 완료
3. `npm run audit:runtime` high gate 상향과 만료형 allowlist 구조 고정: 완료
4. disposable DB 기반 `prisma-integration` 첫 GitHub 통과 증적 확보: 완료
5. 외부 감사 저장소/장기 보관 정책 초안 정리: 남음

남은 항목은 운영 배포 리허설과 장기 감사 보관 정책처럼 실제 운영 단계에서 확정할 성격이 큽니다.

2026-04-05 업데이트 이력:

- runtime dependency audit를 다시 실행했고, 결과는 `critical 0`, `high 4`였습니다.
- 당시 남은 4건은 모두 `@nestjs/config 4.0.3`, `@nestjs/swagger 11.2.6`이 내부에서 `lodash 4.17.23`, `path-to-regexp 8.3.0`을 exact dependency로 유지하는 데서 왔습니다.
- 따라서 당시 라운드의 판정은 “즉시 로컬 업그레이드 불가, tracked exception으로 문서화”였습니다.

2026-04-22 업데이트:

- runtime dependency audit를 다시 실행했고, 새로 `next 15.5.14` high advisory까지 포함해 총 `high 5`, `critical 0` 상태임을 확인했습니다.
- `@nestjs/config 4.0.4`, `@nestjs/swagger 11.3.2`, `next 15.5.15`로 패치 업데이트했습니다.
