...
이 저장소는 `실제 MSA`가 아니라 `MSA 대비 설명이 가능한 modular monolith`입니다.
즉 service 분리, broker, service별 DB를 도입하지는 않지만, 도메인 책임 경계는 아래처럼 읽히도록 유지합니다.
...
| 사업장 운영 관리    | `workspace-settings`, `admin`, `navigation`, `common/infrastructure/operational`                                                                                                                                                            | 사업장 설정, 멤버/권한 정책, 감사 이벤트 저장과 조회, DB 메뉴 트리                                              |
...
- 다른 모듈의 repository, adapter, controller를 직접 가져다 쓰는 것을 기본 규칙으로 두지 않습니다.
- `packages/contracts`에 앱 구현 코드나 비즈니스 로직을 넣지 않습니다.
- 메시지 broker, outbox, gateway, service 분리는 별도 ADR 없이 도입하지 않습니다.

## 3.1 금액 정합성 경계

- HTTP 요청/응답의 금액은 `MoneyWon` 의미의 `number`이며 KRW 원 단위 안전한 정수만 허용합니다.
- Prisma 영속 금액 컬럼은 `Decimal(19,0)`로 유지하고, 비즈니스 로직 경계에는 mapper/adapter의 `Prisma.Decimal -> MoneyWon(number)` 변환을 거쳐 전달합니다.
- 금액 덧셈, 차감, 합계, `HALF_UP` 반올림, 배분 잔차 보정은 `@personal-erp/money` 헬퍼 함수를 우선 사용합니다.
...
shared/    # 공통 UI, 레이아웃, 테마, env, fetch 헬퍼 함수
...
모듈이 `기본 방식`(service 중심)에서 `고급 방식`(헥사고날 중심)으로 전환할 후보인지 판단할 때 아래 신호를 봅니다.
한 항목만 만족한다고 자동으로 승격하지는 않고, 비즈니스 중요도와 설명 가치, 테스트 필요성이 함께 높을 때 전환합니다.

1.  **비즈니스 복잡도**
    - 단일 service 파일이 **400줄 이상**으로 길어지고, 정책 분리나 테스트 분리가 실제 이해도를 높일 때.
    - 로직 내부에 중괄호 깊이가 깊거나 복잡한 정책(`Policy`)이 3개 이상 얽힐 때.
2.  **데이터 중요도**
    - **돈(현금, 자산)**의 흐름을 직접 계산하거나 변경하는 로직일 때.
    - 법적/회계적 **증빙(전표, 감사 기록)**이 반드시 필요한 데이터일 때.
3.  **테스트 요구사항**
    - DB 없이 순수 로직만으로 **단위 테스트**를 20개 이상 작성해야 할 만큼 검증이 중요할 때.
4.  **외부 결합도**
...
- controller와 service는 인증된 `user.currentWorkspace`를 받고, 도메인 쓰기 흐름에서는 이를 `tenantId`, `ledgerId`, `TenantMembership`, `ActorRef` 기준으로 해석해야 합니다.
...
`npm run test:e2e`, `npm run test:prisma`는 대표 사용자 흐름과 실제 DB 경계를 따로 보는 심화 검증입니다.
...
- `GET /health`는 생존 확인, `GET /health/ready`는 DB 연결을 포함한 준비 상태를 나타냅니다.
