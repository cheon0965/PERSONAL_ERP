...
- 이 문서는 현재 HTTP 표면과 구현 상태를 설명하며, 도메인 정책을 다시 정의하지 않습니다.
...
- `POST /auth/register`, `POST /auth/verify-email`, `POST /auth/resend-verification`, `POST /auth/accept-invitation`, `POST /auth/forgot-password`, `POST /auth/reset-password`, `POST /auth/login`, `POST /auth/refresh`, `POST /auth/logout`은 브라우저 `Origin` 또는 `Referer`가 allowlist에 없으면 `403 Origin not allowed`를 반환합니다.
...
- 설정 범위는 현재 워크스페이스의 사업장/기본 장부 설정 조회와 Owner/Manager 수정까지 포함합니다.
- 관리자 범위는 현재 워크스페이스의 멤버 목록/초대/역할·상태 관리, DB 메뉴 트리/메뉴 권한 관리, 워크스페이스 감사 로그 조회, 권한 정책 요약까지 포함합니다. `INITIAL_ADMIN_*`로 시드된 전역 관리자는 일반 사업장 관리자와 분리되어 전체 사용자 관리, 사업장 관리, 사업장 전환/지원 문맥, 운영 상태 점검, 보안 위협 로그, 전체 멤버십 역할·상태 관리를 수행할 수 있습니다.
- 내비게이션 범위는 현재 워크스페이스의 DB 메뉴 트리를 현재 멤버 역할 기준으로 필터링해 반환하는 `navigation/tree`까지 포함합니다.
- 운영 지원 범위는 checklist, 예외 처리함, 월 마감 대시보드, 업로드 운영 현황, 시스템 상태, 알림 센터, 수동 CSV 반출, 운영 메모까지 포함합니다.
...
- 즉, 현재 저장소의 API 표면은 초기 reference-data/transactions 수준을 넘어 기준 데이터, 보험/차량 운영 기준, 반복 계획, 수집, 업로드 배치, 전표, 공식 보고, 자금수단별 현황, 차기 이월, 기간 전망까지 포함합니다.
...
- contract: response `AdminOperationsStatus`
- 전역 관리자만 사용자/사업장 수, 잠금 계정, 최근 24시간 보안 위협, 감사 실패/거부, API/DB 점검 상태를 조회합니다.
- Prisma 마이그레이션 상세 비교는 배포 절차에서 별도 확인하며, 이 응답은 운영 점검판 역할을 합니다.

### `GET /admin/members`

- contract: response `AdminMemberItem[]`
...
- contract: `InviteTenantMemberRequest -> TenantMemberInvitationItem`
...
- 초대 성공과 메일 발송 실패는 워크스페이스 감사 이벤트로 기록합니다.

### `POST /auth/accept-invitation`

- contract: `AcceptInvitationRequest -> AcceptInvitationResponse`
...
- contract: response `DeleteWorkspaceResponse`
...
- contract: `SwitchWorkspaceRequest -> SwitchWorkspaceResponse`
...
- contract: `UpdateTenantMemberRoleRequest -> AdminMemberItem`
- 현재 작업 문맥의 Owner만 멤버 역할을 변경할 수 있습니다.
- 마지막 활성 Owner를 다른 역할로 낮추는 요청은 `400 Bad Request`로 막습니다.
- 역할 변경 성공은 워크스페이스 감사 이벤트로 기록합니다.

### `PATCH /admin/members/:membershipId/status`

- contract: `UpdateTenantMemberStatusRequest -> AdminMemberItem`
- 현재 작업 문맥의 Owner만 멤버 상태를 `ACTIVE`, `SUSPENDED`, `REMOVED`로 변경할 수 있습니다.
- 마지막 활성 Owner를 중지 또는 제거 상태로 바꾸는 요청은 `400 Bad Request`로 막습니다.
- 상태 변경 성공은 워크스페이스 감사 이벤트로 기록합니다.

### `DELETE /admin/members/:membershipId`

- contract: response body 없음 (`204 No Content`)
- 현재 작업 문맥의 Owner만 멤버를 하드 삭제하지 않고 `REMOVED` 상태로 전환합니다.
- 마지막 활성 Owner 제거 요청은 `400 Bad Request`로 막습니다.
- 제거 성공은 워크스페이스 감사 이벤트로 기록합니다.

### `GET /admin/audit-events`

- contract: `AdminAuditEventQuery -> AdminAuditEventListResponse`
...
- contract: response `AdminAuditEventItem`
- 현재 작업 문맥의 Owner만 현재 Tenant에 속한 감사 이벤트 상세를 조회할 수 있습니다.

### `GET /admin/security-threats`

- contract: response `AdminSecurityThreatEventListResponse`
...
- contract: `OpenAccountingPeriodRequest -> AccountingPeriodItem`
...
- contract: `CloseAccountingPeriodRequest -> CloseAccountingPeriodResponse`
- 운영 기간을 잠그고 closing snapshot을 생성합니다.
- 공식 `financial-statements`와 `carry-forwards`의 선행 조건입니다.

### `POST /accounting-periods/:id/reopen`

- contract: `ReopenAccountingPeriodRequest -> AccountingPeriodItem`
...
- contract: response `ReferenceDataReadinessSummary`
...
- contract: response `FundingAccountItem[]`
- 현재 작업 문맥 기준 활성 자금수단만 반환합니다.
- `?includeInactive=true`를 주면 비활성/종료 자금수단까지 함께 반환합니다.
- 응답의 `bootstrapStatus`는 신규 계좌/카드의 시작 잔액 초기화가 아직 닫히지 않았는지 표시합니다. `PENDING`인 활성 `BANK`/`CARD`만 업로드 배치의 신규 계좌/카드 초기화 후보가 될 수 있습니다.

### `POST /funding-accounts`

- contract: `CreateFundingAccountRequest -> FundingAccountItem`
...
- contract: `CompleteFundingAccountBootstrapRequest -> FundingAccountItem`
...
- contract: `UpdateFundingAccountRequest -> FundingAccountItem`
...
- contract: response `CategoryItem[]`
- 현재 작업 문맥 기준 활성 카테고리만 반환합니다.
- `?includeInactive=true`를 주면 비활성 카테고리까지 함께 반환합니다.

### `POST /categories`

- contract: `CreateCategoryRequest -> CategoryItem`
...
- contract: `UpdateCategoryRequest -> CategoryItem`
...
- contract: response `InsurancePolicyItem[]`
- 현재 작업 문맥 기준 활성 보험 계약만 반환합니다.
- `?includeInactive=true`를 주면 비활성 보험 계약까지 함께 반환합니다.

### `POST /insurance-policies`

- contract: `CreateInsurancePolicyRequest -> InsurancePolicyItem`
...
- contract: `UpdateInsurancePolicyRequest -> InsurancePolicyItem`
...
- contract: `UpdateLiabilityAgreementRequest -> LiabilityAgreementItem`
...
- contract: response `VehicleOperatingSummaryView`
...
- contract: `CreateVehicleRequest -> VehicleItem`
...
- contract: `UpdateVehicleRequest -> VehicleItem`
...
- contract: response `VehicleFuelLogItem[]`
- 현재 작업 문맥 기준 차량 연료 이력을 워크스페이스 범위로 모아 반환합니다.
...
- contract: `CreateVehicleFuelLogRequest -> VehicleFuelLogItem`
...
- contract: `UpdateVehicleFuelLogRequest -> VehicleFuelLogItem`
...
- contract: response `VehicleMaintenanceLogItem[]`
- 현재 작업 문맥 기준 차량 정비 이력을 워크스페이스 범위로 모아 반환합니다.
- 정비 이력은 차량 기본 정보 응답과 분리된 별도 운영 기록 모델이며, 차량명과 `linkedCollectedTransaction` 요약은 read model 편의를 위해 함께 내려갑니다.
- `linkedCollectedTransaction`이 있으면 차량 화면에서 회계 연동 상태, 연결 수집거래 ID, 자금수단, 카테고리, 전표 확정 여부를 확인할 수 있습니다.

### `POST /vehicles/:id/maintenance-logs`

- contract: `CreateVehicleMaintenanceLogRequest -> VehicleMaintenanceLogItem`
...
- contract: `UpdateVehicleMaintenanceLogRequest -> VehicleMaintenanceLogItem`
...
- contract: `CreateCollectedTransactionRequest -> CollectedTransactionItem`
...
- contract: response `CollectedTransactionDetailItem`
...
- contract: `UpdateCollectedTransactionRequest -> CollectedTransactionItem`
...
- contract: request body 없음 -> `JournalEntryItem`
...
- contract: `BulkConfirmCollectedTransactionsRequest -> BulkConfirmCollectedTransactionsResponse`
...
- contract: `CreateRecurringRuleRequest -> RecurringRuleItem`
- 반복 규칙을 등록합니다.
- 현재 엔드포인트는 도메인 기준의 `RecurringRule -> PlanItem` 생성 흐름의 입력 단계 구현입니다.

### `GET /recurring-rules/:id`

- contract: response `RecurringRuleDetailItem`
- 특정 반복 규칙의 상세 값을 읽습니다.
- 현재 Web 드로어 수정 흐름은 이 상세 응답을 기준으로 초기값을 채웁니다.

### `PATCH /recurring-rules/:id`

- contract: `UpdateRecurringRuleRequest -> RecurringRuleItem`
...
- contract: `GeneratePlanItemsRequest -> GeneratePlanItemsResponse`
...
- contract: `CreateImportBatchRequest -> ImportBatchItem`
...
- contract: `multipart/form-data(sourceKind, fundingAccountId, file, password?) -> ImportBatchItem`
...
- contract: request body 없음 -> `CancelImportBatchCollectionResponse`
...
- contract: `CollectImportedRowRequest -> CollectImportedRowPreview`
...
- contract: `CollectImportedRowRequest -> CollectImportedRowResponse`
...
- 운영월 자동 생성은 일반 거래 입력 예외가 아니라 초기화 지원입니다. 아직 운영 기간이 없는 최초 시작월 또는 최신 잠금 월 바로 다음 월에 대한 신규 활성 계좌/카드 초기화일 때만 허용합니다.
...
- 신규 계좌/카드 초기화 수집은 해당 자금수단이 `bootstrapStatus=PENDING`이고 기존 수집거래, 다른 import batch, 전표 line, opening/closing snapshot line이 없을 때만 열립니다. 첫 수집 성공 후에는 `COMPLETED`로 전환됩니다.
- 같은 업로드 행 재수집은 `importedRowId` 기준으로 막고, 반복 수집 거래 흡수는 조건부 claim으로 덮어쓰기를 방지합니다.
- 일괄 등록 Job이 같은 워크스페이스에서 실행 중이면 단건 collect도 같은 배치/다른 배치 여부에 따라 `409 Conflict`로 막습니다.
- 중복/경합은 `409 Conflict`로 정리합니다.

### `POST /import-batches/:id/rows/collect`

- contract: `BulkCollectImportedRowsRequest -> BulkCollectImportedRowsResponse`
...
- contract: request body 없음 -> `ImportBatchCollectionJobItem`
...
- contract: `CancelCarryForwardRequest -> CancelCarryForwardResponse`
...
- contract: response `FundingAccountOverviewResponse | null`
...
- 현재 구현은 단순 user-scoped 전단계가 아니라 워크스페이스-scoped tenant/ledger 모델을 사용합니다.
- 조회 엔드포인트는 인증된 워크스페이스 범위 내 데이터만 반환합니다.
- `insurance-policies`, `liabilities`, `vehicles`도 개인 생활용 고정 데이터가 아니라 현재 워크스페이스/원장 기준 사업 운영 보조 자산 데이터만 반환합니다.
- 쓰기 권한은 워크스페이스 membership role로 제어합니다.
