...
- 현재 상태: 비밀번호 기반 로그인, common/컨텍스트-derived password 차단, Bearer 보호 엔드포인트, generic login failure, login/refresh rate limit
- 근거 파일:
  `apps/api/src/modules/auth/domain/password-policy.ts`
...
- 근거 파일:
  ...
  `apps/api/test/request-api/auth/*.test.ts`
- 판정: `부분 적용`
- 다음 단계: 배포 설정 변경 시 cookie 속성과 session revoke/reuse event 증적 재확인

### 접근통제

- 현재 상태: 전역 guard, 워크스페이스 기반 tenant/ledger/membership role 검증, 목록/집계 응답의 current 워크스페이스 범위 분리
- 근거 파일:
  `apps/api/src/common/auth/jwt-auth.guard.ts`
...
- 현재 상태: CORS allowlist, API/Web security headers, CSP, HSTS, `no-store`, 브라우저 origin allowlist 적용
- 근거 파일:
...
- 판정: `적용`
- 다음 단계: 배포 설정 변경 시 health/HSTS와 Swagger 토글 상태 증적 재확인
...
- 근거 파일:
  `apps/api/src/config/api-env.ts`
  `apps/api/test/unit/config/api-env.test.ts`
  `env-examples/secret-dir.local.example`
  `env-examples/api.env.example`
  `env-examples/web.env.example`
  `README.md`
...
- 현재 상태: 회원가입, 이메일 인증, 비밀번호 재설정이 구현됨. 주요 비밀번호 설정 경로에 common/컨텍스트-derived password 차단 정책이 적용됨
...
1. 운영 HTTPS/HSTS와 Swagger 토글은 현재 공개 배포 기준에 반영했으며, 배포 설정 변경 시 증적을 다시 확인해야 한다.
2. 보안 이벤트는 남기기 시작했지만 외부 감사 저장소나 장기 보관 정책은 아직 없다.
3. 운영 데이터 분류와 장기 보관 정책은 실제 배포 환경 기준으로 더 구체화해야 한다.

## 최근 해소된 갭

1. 런타임 의존성 audit의 2026-04-05 tracked exception은 2026-04-22 패치 업데이트로 해소되었고, 같은 날 `high` gate와 만료형 allowlist 구조로 상향되었다.
...
2. 워크스페이스 접근통제가 핵심 쓰기 흐름과 읽기 응답 분리에 모두 반영되어 있다.
...
- 근거는 `GET /collected-transactions`, `GET /recurring-rules`, `GET /dashboard/summary`, `GET /forecast/monthly`에 대한 요청 단위 API 테스트에서 current 워크스페이스 범위만 반환하고 내부 접근 제어 필드를 노출하지 않음을 검증한 점이다.
- 신규 자금수단별 현황 read model인 `GET /funding-account-status/summary`도 같은 projection 원칙으로 동작하며, 전용 요청 단위 회귀 테스트에서 기간/자금수단 필터, current 워크스페이스 범위, 합계, 거래 목록, 경고 메시지를 검증한다.
