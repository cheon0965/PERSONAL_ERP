...
실제 제거 PR에서 손대야 할 스키마/code touchpoint 체크리스트는 [`docs/LEGACY_TRANSACTION_SCHEMA_REMOVAL_CHECKLIST.md`](./LEGACY_TRANSACTION_SCHEMA_REMOVAL_CHECKLIST.md)에 따로 고정한다.
...
| `apps/api/test/collected-transactions.prisma.integration.test.ts` | 실DB 통합 테스트에서 backbone 초기화 재사용                           | 신규 수집 거래 흐름 검증 전에 워크스페이스/원장 기반선을 일관되게 맞춤     |
| `apps/api/test/prisma-integration.test-support.ts`                | 실DB request/API 테스트 초기화 재사용                                 | 테스트 컨텍스트를 현재 워크스페이스/원장 구조 기준으로 맞춤                |
...
현재 기준으로 `apps/api/src`, `apps/web/src`, `packages/contracts`에는 `LegacyTransaction` direct 런타임 의존성을 허용하지 않는다.
...
3. `apps/api/src`, `apps/web/src`, `packages/contracts` 기준 direct 의존성이 계속 0인지 경계 테스트와 코드 검색으로 확인한다.
...
1. 대상 환경에 `npm run db:backfill:phase1`를 실행해 레거시 rows의 워크스페이스/원장 정합성을 먼저 맞춘다.
2. 제거 대상 환경에서 현재 검증 기준(`check:quick`, `test:api`, 필요 시 `test:prisma`)을 다시 통과시킨다.
3. `apps/api/prisma/schema.prisma`에서 `LegacyTransaction` 모델과 관련 relation을 제거하는 마이그레이션을 별도 PR로 만든다.
4. 마이그레이션과 같은 변경에서 boundary 테스트, seed 초기화, 문서 링크를 새 기준으로 같이 정리한다.
5. Prisma client regenerate 후 애플리케이션 검증을 다시 수행한다.

핵심은 `LegacyTransaction` 제거를 신규 기능 변경과 섞지 않고, "스키마 제거 + 문서/테스트 동기화" PR로 따로 다루는 것이다.
...
- `phase1-backbone` import 표면이 seed/backfill/테스트 초기화 허용 지점으로만 제한돼 있다.
- 스키마 제거 PR에서 지워야 할 relation/model/code touchpoint가 별도 checklist 문서로 정리돼 있다.
- 삭제 순서와 rollback 기준이 문서로 설명 가능하다.
- 제거 PR이 들어오면 스키마 마이그레이션과 boundary 문서/테스트 정리만 집중해서 진행할 수 있다.
