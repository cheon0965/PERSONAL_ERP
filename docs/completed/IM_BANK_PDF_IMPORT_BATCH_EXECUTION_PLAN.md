...
추천 설정은 "한 파일은 하나의 원본 배치" 원칙이다. 샘플 PDF처럼 2년치 거래가 들어 있어도 배치는 전체 565행을 보관하고, 작업대에서 현재 운영월/기간 필터로 검토 범위를 좁히는 방식이 추적성과 재업로드 판정에 가장 깔끔하다. 현재 정책에서는 이 중 최신 진행월 범위의 행만 수집 거래로 승격하며, 운영월 자동 생성은 운영 시작 전 기초데이터 또는 신규 계좌/카드 기초 업로드 예외로 제한한다.
...
기존 화면과 collect use case는 `parsed.occurredOn`, `parsed.title`, `parsed.amount`를 읽고 있으므로 이 3개 필드는 반드시 유지한다. PDF 전용 부가 필드는 선택적으로 표시한다.
...
| 제목 후보가 모두 공백              | `IM뱅크 입금` 또는 `IM뱅크 출금` 같은 대체 제목 생성                          |
...
- Prisma `ImportSourceKind` enum에도 같은 값을 추가하고 마이그레이션을 만든다.
...
샘플 PDF 기준 컬럼 좌표는 대략 다음 범위다. 실제 구현은 고정값만 쓰지 말고 헤더 위치를 먼저 감지한 뒤 대체값으로 사용한다.
...
- 계좌번호 원문은 저장하지 않고 `sha256(tenantId + ledgerId + accountNumber)` 같은 워크스페이스 범위 해시만 메타데이터에 남긴다.
...
- 제목 공백 행은 대체 제목으로 `PARSED` 처리되는지 확인한다.
...
2. `ImportBatchParser` interface와 기존 delimited parser 분리
