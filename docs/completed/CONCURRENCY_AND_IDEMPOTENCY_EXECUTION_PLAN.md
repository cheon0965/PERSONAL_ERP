...
- `apps/api/src/modules/collected-transactions/application/use-cases/confirm-collected-transaction.use-case.ts`
- `apps/api/src/modules/journal-entries/application/use-cases/reverse-journal-entry.use-case.ts`
- `apps/api/src/modules/journal-entries/application/use-cases/correct-journal-entry.use-case.ts`
...
- `apps/api/src/modules/import-batches/infrastructure/services/imported-row-collection.service.ts`
...
- `apps/api/src/modules/plan-items/application/use-cases/generate-plan-items.use-case.ts`
...
- 현재 API 표면과 응답 계약은 `docs/API.md`와 `packages/contracts`를 우선한다.
...
- 중복 기준이 문서와 스키마, service 로직에서 서로 다르게 해석되지 않는다.
...
- request/integration 테스트 명령을 CI 또는 로컬 검증 루프에 연결한다.
