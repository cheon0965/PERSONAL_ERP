...
- `코드 + test + 문서`가 함께 따라오는 순서로 구현해 이후 운영 문서와 화면이 드리프트되지 않게 한다.
...
- Phase 2: 완료. 운영 허브, checklist, 예외 처리함, 월 마감 대시보드, 업로드 운영 현황을 `operations-console` read model로 연결했습니다.
...
- 커스텀 Role Builder, 플랫폼급 멀티테넌시 운영 UI, 별도 메시지 broker, 중앙 SIEM, 장기 보관용 백업 오케스트레이터는 이번 범위에 넣지 않는다.
...
| 5    | 예외 처리함          | `/operations/exceptions` | 실제 운영에서 가장 자주 보는 액션 허브이며 이후 대시보드의 공통 요약 소스가 됨  | checklist, 예외 집계 read model                |
...
- checklist, 예외 처리함, 월 마감 대시보드, 업로드 운영 현황이 중복 계산 없이 같은 요약 소스를 사용하게 한다.
...
- 공통 계약과 라우트 naming을 먼저 확정한다.
- `docs/API.md`, `docs/VALIDATION_NOTES.md`에 이후 추가될 표면 범주를 반영할 준비를 한다.
...
- 현재 `작업 문맥` 화면은 read-only 홈으로 유지하고, 실제 수정은 별도 워크스페이스 하위 화면으로 분리한다.
...
- API: `apps/api/src/modules/admin/*` 또는 static contract 기반 read 엔드포인트
...
- 권한 정책 요약이 먼저 있어야 checklist, 예외 처리함, 알림 센터의 CTA 노출 기준이 일관됩니다.
...
- 모든 관련 표면이 `contracts -> API -> Web -> tests -> docs` 순서를 유지한다.
