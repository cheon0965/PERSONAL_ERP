...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| 인증/설정   | 실제 운영 화면 기반 공개 홍보 메인, 한글 검색어 메타데이터, FAQ/구조화 데이터, 회원가입, 이메일 인증, 인증 메일 재발송, 초대 수락, 로그인, 비밀번호 재설정, refresh/logout, 현재 사용자 조회, 접근 가능 사업장 목록, 세션 단위 사업장 전환, `현재 사업장 / 장부`, 사업장 설정, 계정 보안/프로필/비밀번호/세션/보안 이벤트 | `auth`, `health`                                                                                             |
...
루트 `/`는 비로그인 사용자에게 제품 소개, 실제 운영 대시보드 스크린샷, 데모/로그인/GitHub CTA, 활용 사례, FAQ가 있는 공개 홍보 메인을 보여주고, 이미 인증된 사용자는 `/dashboard`로 이동합니다.
검색 노출은 공개 홈만 대상으로 하며, `apps/web/src/shared/seo/site.ts`의 한글 검색어, Open Graph 이미지, JSON-LD `WebSite`/`WebPage`/`SoftwareApplication`/`FAQPage`, `robots.txt`, `sitemap.xml`, Google Search Console 확인 파일을 함께 유지합니다.
...
- 업무 화면 도움말은 스크롤된 상태에서도 상단 헤더와 현재 스크롤 위치를 유지한 채 열리고 닫히도록 구성했습니다.
