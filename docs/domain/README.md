...
- `../API.md`: API contract과 인증/쓰기 흐름 요약
...
- 비즈니스 로직, use case, 상태 흐름, DB 모델링 논의 전 이 폴더 문서를 먼저 기준으로 본다.
- 구현이 기준과 달라지면 관련 PR에서 이 폴더 문서도 함께 갱신한다.
- 다른 문서가 도메인 정책을 다시 정의하지 않도록, 필요하면 이 폴더 문서를 링크로 참조한다.
- 현재 코드에 실제 구현된 범위는 `../CURRENT_CAPABILITIES.md`와 `../API.md`를 우선한다.
- `TenantSubscription`, `SupportAccessGrant`처럼 SaaS 운영 확장을 설명하는 모델은 코드에 대응 모델이 생기기 전까지 현재 구현 범위가 아니라 설계 기준으로 읽는다.
