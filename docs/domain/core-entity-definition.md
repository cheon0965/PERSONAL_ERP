...
- 신규 `Bank`/`Card` 계정의 `bootstrapStatus=Pending`은 등록 직후 시작 잔액 초기화가 아직 닫히지 않았음을 의미한다. 첫 수집 성공 후에는 `Completed`가 되고, 기존 회계 산출물이 있으면 신규 계좌/카드 bootstrap으로 오인하지 않는다.
...
- 같은 workspace에서는 동시에 하나의 업로드 배치 일괄 등록 Job만 실행한다.
- Job의 행별 결과는 `ImportBatchCollectionJobRow`에 남겨, 성공/실패/건너뜀 사유를 다시 조회할 수 있어야 한다.
- 진행 중 Job이 있으면 같은 workspace의 다른 단건 collect나 일괄 collect는 충돌로 보호한다.
...
workspace 단위 업로드 배치 일괄 등록 동시 실행을 막는 잠금이다.

### 불변조건

- 잠금은 특정 workspace의 활성 일괄 등록 Job 하나를 가리킨다.
...
- 대량 승격은 `ImportBatchCollectionJob`과 workspace 잠금으로 조율하지만, 회계 확정은 여전히 수집 거래별 전표 확정에서 처리한다.
