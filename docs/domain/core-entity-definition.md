...
## 전표 기반 1인 사업자·소상공인 월별 재무운영 ERP starter (SaaS 운영 반영 / 구현 보강)
...
본 시스템은 **1인 사업자와 소상공인의 월별 재무 운영을 계획하고, 실제 사업 거래를 수집·검토·확정하여 전표로 전기하고, 결산과 재무제표 산출 및 차기 이월까지 수행하는 전표 기반 월별 재무운영 ERP starter**다.
...
1인 사업자·소상공인 월별 재무운영 ERP starter의 최상위 회계 단위다.  
...
- 정책상 필요하면 직접 입력 거래도 `Collected` 또는 `Reviewed`를 거칠 수 있다.
