...
- 이후 use case/상태 흐름표/DB 모델링 단계에서 흔들리지 않도록 기준을 고정한다.
...
본 시스템은 **1인 사업자와 소상공인의 월별 재무 운영을 계획하고, 실제 사업 거래를 수집·검토·확정하여 전표로 전기하고, 결산과 재무제표 산출, 차기 이월, 다음 기간 전망까지 수행하는 전표 기반 월별 재무 운영 시스템**이다.

### 2.2 SaaS 전제

본 시스템은 SaaS 운영을 목표로 하므로, 회계 도메인 바깥에 다음 플랫폼 경계가 존재한다.

현재 코드베이스는 `User`, `Tenant`, `Ledger`, `TenantMembership`, `TenantMembershipInvitation`, 전체 관리자 지원 문맥, 워크스페이스 감사 이벤트를 구현 기준으로 둔다.
...
다음은 단독 생명주기를 갖기보다 상위 엔티티에 종속되거나 value object로 다루는 것이 자연스럽다.
...
즉, 재무제표 산출 결과는 중요하지만 **핵심 write-domain 엔티티**로 두기보다 projection/snapshot 관점으로 다룬다.

### 3.4 자동전표 정책의 위치

`PostingPolicy`는 엔티티가 아니라 **도메인 정책/service 객체**로 본다.
...
모든 쓰기 작업이 **누가 어떤 경로로 수행했는지**를 추적하기 위한 value object다.
...
- 최신 운영월이 아직 진행 중이면 다음 Period를 임의로 열 수 없다.
- 새 Period는 최근 운영월을 마감한 뒤 여는 것이 기본이며, 운영 중 수집/전표 입력은 최신 진행월 범위로 제한한다.
...
- `bootstrapStatus` (`NotRequired`, `Pending`, `Completed`)
- `createdByActorRef`
- `openedAt`
- `closedAt`
...
- 신규 `Bank`/`Card` 계정의 `bootstrapStatus=Pending`은 등록 직후 시작 잔액 초기화가 아직 닫히지 않았음을 의미한다. 첫 수집 성공 후에는 `Completed`가 되고, 기존 회계 산출물이 있으면 신규 계좌/카드 초기화로 오인하지 않는다.
...
사업 운영에 사용하는 차량의 기본 프로필이다.
연료/정비 운영 이력과 회계 수집거래의 기준 컨텍스트를 제공하지만, 차량 프로필 자체가 전표는 아니다.
...
- `initialOdometerKm`
- `estimatedFuelEfficiencyKmPerLiter` (nullable)
- `defaultFundingAccountId` (nullable)
- `defaultFuelCategoryId` (nullable)
- `defaultMaintenanceCategoryId` (nullable)
- `operatingExpensePlanOptIn`

### 불변조건

- 같은 Ledger 안에서 normalized 차량명은 중복될 수 없다.
- 차량 프로필 수정은 과거 전표 숫자를 직접 바꾸지 않는다.
- 차량별 기본 자금수단과 카테고리는 연료/정비 입력의 제안값이며, 저장 전 사용자가 바꿀 수 있다.
- 차량 운영비 계획 자동 생성과 백필은 `operatingExpensePlanOptIn=true`인 차량만 대상으로 삼는다.
- 차량별 운영비/연비는 프로필 필드가 아니라 연료/정비 이력 기반 read model에서 계산한다.
...
차량 연료 주입 또는 충전 1건을 나타내는 운영 이력이다.
사용자가 회계 연동을 켠 경우 표준 지출 `CollectedTransaction`과 1:1로 연결될 수 있다.
...
- `unitPrice`
- `isFullTank`
- `linkedCollectedTransactionId` (nullable)

### 불변조건

- 회계 연동이 없는 연료 기록은 운영 이력으로만 남는다.
- 회계 연동을 켠 연료 기록의 금액은 0보다 커야 하며, 거래일이 최신 진행월 범위 안에 있어야 한다.
- 연결 수집거래가 미확정 상태이면 차량 연료 기록 수정 시 함께 동기화하거나 삭제 시 함께 정리할 수 있다.
- 연결 수집거래가 `Posted`, `Corrected`, `Locked` 상태이면 연료 기록 overwrite/delete를 막고 전표 조정 흐름을 사용한다.

---

## 6.6-3 VehicleMaintenanceLog

### 역할

차량 정비 1건을 나타내는 운영 이력이다.
사용자가 회계 연동을 켠 경우 표준 지출 `CollectedTransaction`과 1:1로 연결될 수 있다.
...
- 회계 연동이 없는 정비 기록은 운영 이력으로만 남는다.
- 회계 연동을 켠 정비 기록의 금액은 0보다 커야 하며, 거래일이 최신 진행월 범위 안에 있어야 한다.
- 연결 수집거래가 미확정 상태이면 차량 정비 기록 수정 시 함께 동기화하거나 삭제 시 함께 정리할 수 있다.
- 연결 수집거래가 `Posted`, `Corrected`, `Locked` 상태이면 정비 기록 overwrite/delete를 막고 전표 조정 흐름을 사용한다.

---

## 6.6-4 LiabilityAgreement

### 역할

은행 대출, 카드론, 금융리스성 차입처럼 월별 상환이 발생하는 부채 계약의 운영 기준 데이터다.
부채 계약은 공식 회계 잔액을 직접 대체하지 않고, 상환 일정과 계획/수집/전표 연결의 기준을 제공한다.
...
- `feeExpenseCategoryId` (nullable)
- `status` (`Active`, `PaidOff`, `Archived`)
- `memo` (nullable)

### 불변조건

- 부채 계약은 Tenant/Ledger 경계 안에서만 조회·수정되어야 한다.
- 대출번호는 전체 원문보다 마지막 4자리 같은 최소 식별값 중심으로 보관한다.
- 기본 출금 자금수단과 부채 계정과목은 상환 계획과 전표 정책의 기준이 된다.
- 공식 부채 잔액은 `JournalEntry`와 마감 산출물이 기준이며, 계약 화면의 금액은 운영 보조 기준이다.

---

## 6.6-5 LiabilityRepaymentSchedule

### 역할

특정 부채 계약에서 특정 날짜에 상환할 예정 금액이다.
상환 총액을 원금, 이자, 수수료로 분리해 계획 항목과 수집 거래, 최종 전표 확정까지 연결한다.

### 필수 속성

- `liabilityRepaymentScheduleId`
- `tenantId`
- `ledgerId`
- `liabilityAgreementId`
- `dueDate`
...
- `feeAmount`
- `totalAmount`
- `status` (`Scheduled`, `Planned`, `Matched`, `Posted`, `Skipped`, `Cancelled`)
- `linkedPlanItemId` (nullable)
- `postedJournalEntryId` (nullable)
- `memo` (nullable)

### 불변조건

- 같은 부채 계약 안에서 동일 상환 예정일은 중복될 수 없다.
- `totalAmount`는 원금, 이자, 수수료 합계와 일치해야 한다.
- 계획 항목 금액은 현금 유출 기준 총액이며, 손익 비용은 전표 확정 시 이자/수수료만 반영한다.
- `Matched` 상태는 계획 항목에 연결된 수집 거래가 있음을 뜻하고, 업로드 배치 취소 시 `Planned`로 복구한다.
- `Posted` 상태 상환 예정은 기존 전표를 덮어쓰지 않고 반전/정정 흐름으로 조정해야 한다.
...
특정 월에 생성된 **계획 항목(운영 초안 항목)**이다.
반복 규칙 또는 운영 계획에서 파생되며, 실제 거래와 매칭되어 종료된다.
현재 Web 화면에서는 이 엔티티를 `계획 항목`으로 표시한다.
...
- `linkedLiabilityRepaymentScheduleId` (nullable, 부채 상환 예정에서 생성된 경우)
- `postedJournalEntryId` (nullable)
- `note`
...
- PlanItem은 전표가 아니다.
- 같은 규칙으로 동일 월에 중복 계획 항목이 생성되면 안 된다.
- 부채 상환 예정에서 생성된 계획 항목은 상환 총액 기준 현금 유출 계획으로 표시한다.
...
UTF-8 텍스트/CSV/카드 엑셀 계열 업로드 또는 IM뱅크 PDF 파일첨부 같은 외부 거래 파일 수집의 단위 배치다.
...
- `tenantId`
- `ledgerId`
- `sourceType` (`CardExcel`, `BankCsv`, `ManualUpload`, `ImBankPdf`)
- `uploadedByActorRef`
- `sourceFileName`
- `sourceFileHash`
- `fundingAccountId` (nullable, IM뱅크 PDF와 일괄 등록에서는 활성 계좌/카드 연결 필요)
- `importedAt`
- `rowCount`
- `successCount`
- `errorCount`
...
- IM뱅크 PDF 원본 파일 자체는 저장하지 않고, 파일명/해시/행 단위 원본 payload와 파싱 결과만 보존한다.
- IM뱅크 PDF는 텍스트 레이어가 있는 원본 PDF만 지원한다. 스캔/이미지 PDF는 OCR 미도입 상태로 명시 차단하고, 일반 행 파싱 실패와 구분한다.
...
- 같은 현재 배치 안의 미승격 행끼리는 기존 거래 중복 후보로 보지 않고, 수기 입력 또는 다른 배치에서 이미 만들어진 수집 거래와 겹칠 때 중복 확인 대상으로 본다.

---

## 6.10-1 ImportBatchCollectionJob

### 역할

업로드 배치 작업대에서 여러 `ImportedRow`를 수집 거래로 등록하는 백그라운드 실행 단위다.

회계 전표 확정 Job이 아니라 `CollectedTransaction` 생성/흡수 Job이다.

### 필수 속성

- `importBatchCollectionJobId`
- `tenantId`
- `ledgerId`
- `importBatchId`
...
- 같은 워크스페이스에서는 동시에 하나의 업로드 배치 일괄 등록 Job만 실행한다.
- Job의 행별 결과는 `ImportBatchCollectionJobRow`에 남겨, 성공/실패/건너뜀 사유를 다시 조회할 수 있어야 한다.
- 진행 중 Job이 있으면 같은 워크스페이스의 다른 단건 collect나 일괄 collect는 충돌로 보호한다.

---

## 6.10-2 ImportBatchCollectionLock

### 역할

워크스페이스 단위 업로드 배치 일괄 등록 동시 실행을 막는 잠금이다.

### 불변조건

- 잠금은 특정 워크스페이스의 활성 일괄 등록 Job 하나를 가리킨다.
- Job이 끝나면 잠금은 해제되어야 한다.
- 오래된 잠금은 만료 시각과 heartbeat 기준으로 복구 가능해야 한다.
...
- `tenantId`
- `ledgerId`
- `sourceKind` (`Import`, `Manual`, `SystemGenerated`, `VehicleLog`, `LiabilityRepayment`)
...
- `postedJournalEntryId` (nullable)
- `linkedFuelLogId` 또는 `linkedMaintenanceLogId` (nullable, 차량 운영 이력에서 생성된 경우)
- `linkedLiabilityRepaymentScheduleId` (nullable, 부채 상환 예정 계획에서 생성된 경우)
- `createdByActorRef`
...
- 차량 연료/정비 이력에서 생성된 CollectedTransaction은 해당 차량 이력과 1:1 소유 관계를 가져야 하며, 일반 수집거래 화면에서 직접 수정/삭제하지 않는다.
- 부채 상환 예정에서 생성된 CollectedTransaction은 원금/이자/수수료 분리 전표 정책을 사용하며, 일반 지출 거래로 덮어쓰지 않는다.
...
- 차량 연료/정비 연결 거래는 Category가 있으면 `ReadyToPost`, Category가 없으면 `Reviewed`에서 시작한다.
...
- 현재 구현 정책은 opening balance snapshot 전용 이월이다. `createdJournalEntryId`가 `null`인 상태는 미완료가 아니라 의도된 설계이며, 별도 `CARRY_FORWARD` 전표 정책은 미래 ADR/실행계획에서 결정한다.
...
회계 금액의 통화, 저장 단위, 비교 규칙을 일관되게 다루기 위한 value object다.
...
- `Ledger` 1 - N `Vehicle`
- `Vehicle` 1 - N `FuelLog`
- `Vehicle` 1 - N `VehicleMaintenanceLog`
- `Ledger` 1 - N `LiabilityAgreement`
- `LiabilityAgreement` 1 - N `LiabilityRepaymentSchedule`
- `Ledger` 1 - N `RecurringRule`
- `Ledger` 1 - N `AccountingPeriod`
- `RecurringRule` 1 - N `PlanItem`
- `ImportBatch` 1 - N `ImportedRow`
- `ImportBatch` 1 - N `ImportBatchCollectionJob`
- `ImportBatchCollectionJob` 1 - N `ImportBatchCollectionJobRow`
- `ImportBatch` 1 - N `CollectedTransaction`
- `FuelLog` 0..1 - 1 `CollectedTransaction`
- `VehicleMaintenanceLog` 0..1 - 1 `CollectedTransaction`
- `LiabilityRepaymentSchedule` 0..1 - 1 `PlanItem`
- `LiabilityRepaymentSchedule` 0..1 - 1 `JournalEntry`
...
비즈니스 로직 설계 초안 기준으로 보면, **업로드 수집 / 거래유형별 자동전표 / 재무제표 산출 / 차기 이월 / 기간 전망 read model**은 핵심 흐름이므로 관련 엔티티와 projection은 1차 범위에 포함한다.
...
- 자동전표 로직은 도메인 service/정책 객체에서 구현한다.
...
- 이 경계가 흔들리면 전표는 생성됐는데 원거래 상태가 남거나, 계획 항목만 종료되는 식의 불일치가 생길 수 있다.
...
- 동일 배치 안의 미승격 행끼리만으로 중복 후보를 만들지 않고, 이미 존재하는 수집 거래와 겹칠 때 사용자 확인을 요구한다.
...
- 부채 상환 예정에서 생성된 `PlanItem`도 같은 자동 매칭 후보로 쓰되, 매칭/취소 시 `LiabilityRepaymentSchedule` 상태를 함께 전환한다.
- 대량 승격은 `ImportBatchCollectionJob`과 워크스페이스 잠금으로 조율하지만, 회계 확정은 여전히 수집 거래별 전표 확정에서 처리한다.

### 10.14 차량 운영 이력과 수집거래 소유 경계를 분리한다

- `FuelLog`와 `VehicleMaintenanceLog`는 차량 운영 사실의 원천 기록이다.
- 회계 연동을 켠 차량 운영 이력은 표준 `CollectedTransaction`을 생성하지만, 연결 수집거래의 수정/해제 진입점은 차량 화면으로 제한한다.
- 미확정 연결 수집거래는 차량 운영 이력 수정과 같은 transaction 경계에서 동기화한다.
- 전표 확정 후에는 차량 운영 이력 overwrite로 회계 숫자를 바꾸지 않고, `JournalEntry` 반전/정정 흐름으로 조정 이력을 남긴다.

### 10.15 부채 상환은 일반 지출 확정과 분리한다

- `LiabilityRepaymentSchedule`은 운영 기준 데이터이고, 공식 부채 잔액은 전표 기준으로 판단한다.
- 부채 상환 수집거래를 확정할 때 원금은 부채 감소, 이자/수수료는 비용으로 분리한다.
- 상환 예정 총액과 수집 거래 금액이 맞지 않으면 자동 전표 확정을 차단한다.
- 확정 후 상환 예정 상태는 `Posted`가 되며, 이후 수정은 기존 전표 반전/정정 정책을 따른다.
...
- 확정된 결과를 재무제표, 차기 이월, 다음 기간 전망으로 연결한다.
