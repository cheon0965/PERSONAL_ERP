...
이 프로젝트는 실제 microservice로 운영되는 시스템이 아니라, `실무형 modular monolith`를 목표로 하는 포트폴리오입니다.
...
- 이 프로젝트는 계속 `하나의 저장소`, `하나의 배포 단위`, `하나의 DB`를 유지하는 modular monolith로 둡니다.
- 공식 컨텍스트 map은 아래 여섯 개로 읽습니다.
  - `Identity & Access`
  - `Ledger`
...
- `Identity & Access`는 인증과 요청 주체 기준선만 제공하는 cross-cutting 컨텍스트로 둡니다.
...
- 메시지 broker, outbox, gateway, service split은 별도 ADR 없이 도입하지 않습니다.
...
- 새로운 구조 시도를 할 때마다 이 컨텍스트 map과 충돌하는지 확인해야 합니다.
- 일부 구현은 편하더라도, 다른 모듈 내부 구현을 직접 참조하는 빠른 우회가 제한됩니다.

## 후속 원칙

- 컨텍스트 map이 바뀌면 `PORTFOLIO_ARCHITECTURE_GUIDE.md`와 `docs/ARCHITECTURE.md`를 함께 갱신합니다.
- service 분리, broker, outbox, gateway 같은 실제 MSA 기술 도입은 별도 ADR 없이는 진행하지 않습니다.
