...
- API shape, validation, env, 대체 응답 정책이 바뀌면 관련 문서를 같은 PR에서 함께 갱신합니다.

## 결과

좋은 점:

- 계약 drift가 생겨도 어디서 먼저 확인해야 하는지 즉시 판단할 수 있습니다.
- README, API 문서, 검증 메모, 계획 문서가 서로 역할 충돌을 일으키지 않습니다.
- PR 리뷰에서 문서 누락을 체크하기 쉬워집니다.

비용:

- 작은 변경도 `contract`, Swagger, 테스트, 문서를 함께 보는 습관이 필요합니다.
