...
| 데모 비밀번호 | `Demo1234!`                               |
| 검색 노출     | 공개 홈(`/`), `robots.txt`, `sitemap.xml` |

Docker 배포 런타임은 Web `127.0.0.1:3100`, API `127.0.0.1:4100`을 Caddy가 단일 HTTPS 도메인으로 프록시합니다.
검색 엔진에는 공개 홈(`/`)만 노출 대상으로 두고, 인증 뒤 업무 화면은 `noindex` 정책을 유지합니다.
공개 홈은 실제 운영 대시보드 스크린샷, 개인사업자/소상공인 한글 검색 의도, FAQ, JSON-LD 구조화 데이터를 함께 제공하며 Google Search Console 확인 파일은 `apps/web/public/google827b2fac60b63022.html`에 둡니다.
...
| 공개/검색   | 실제 운영 화면 기반 공개 홈, 한글 검색어 메타데이터, 활용 사례, FAQ, robots/sitemap   |
...
packages/contracts
              화면과 서버가 공유하는 request/response contract
packages/money
              원화 금액 계산 기준
