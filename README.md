...
1인 사업자와 소상공인이 `기준 데이터 준비 -> 월 운영 -> 전표 확정 -> 월 마감 -> 공식 보고 -> 자금수단별 현황 -> 차기 이월 -> 다음 달 전망`까지 한 흐름으로 운영할 수 있도록 만든 워크스페이스형 월별 재무 운영 시스템입니다.
...
| 영역        | 현재 구현 범위                                                                                                                                                                                                                            |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 인증/설정   | 회원가입, 이메일 인증, 인증 메일 재발송, 초대 수락, 로그인, 비밀번호 재설정, 토큰 재발급/로그아웃, `auth/me`, 접근 가능 사업장 목록, 세션 단위 사업장 전환, `현재 사업장 / 장부`, 사업장 설정, 계정 보안/프로필/비밀번호/세션/보안 이벤트 |
...
| 보고 / 판단 | 대시보드, 재무제표 스냅샷, 자금수단별 현황, 차기 이월, 기간 전망                                                                                                                                                                          |
...
`인증 -> 현재 사업장 / 장부 확인 -> 운영 허브 점검 -> 기준 데이터 준비 -> 월 운영 시작 -> 보험/부채/차량 운영 정리 -> 반복 규칙 -> 계획 항목 -> 업로드 배치 또는 수집 거래 -> 전표 확정/반전/정정 -> 월 마감 -> 재무제표 -> 자금수단별 현황 -> 차기 이월 -> 기간 전망`
...
- 인증 기본 정책: `health`, `health/ready`, `auth/register`, `auth/verify-email`, `auth/resend-verification`, `auth/accept-invitation`, `auth/login`, `auth/refresh`, `auth/logout`, `auth/forgot-password`, `auth/reset-password`를 제외한 API는 기본적으로 보호합니다.
- 운영 신호: 모든 API 응답에 `x-request-id` 헤더를 붙이고, 브라우저에서도 해당 헤더를 읽을 수 있게 CORS 노출 헤더로 공개하며, `GET /api/health/ready`로 준비 상태 응답을 제공합니다.
...
- 자금수단별 현황은 운영 판단용 수집 거래 기준과 공식 확인용 확정 전표 기준을 분리해 통장/카드/현금별 수입, 지출, 이체, 잔액 흐름을 보여주는 read model입니다.
