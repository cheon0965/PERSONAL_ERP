...
| 영역           | 현재 구현 범위                                                                                                                                                                                                                             |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 작업 문맥/인증 | 회원가입, 이메일 인증, 인증 메일 재발송, 초대 수락, 로그인, refresh/logout, `auth/me`, 계정 보안/프로필/비밀번호/세션/보안 이벤트                                                                                                          |
| 관리자/권한    | 사업장 설정, 멤버 초대/역할 변경/상태 변경/제거, 메뉴 트리/권한 관리, 감사 로그 조회, 권한 정책 요약                                                                                                                                       |
| 운영 지원      | 체크리스트, 예외 처리함, 월 마감 지원, 업로드 현황, 시스템 상태, 알림 센터, UTF-8 CSV 반출, 운영 메모                                                                                                                                      |
| 기준 데이터    | readiness 요약, 자금수단 lifecycle, 수입/지출 카테고리 관리, 계정과목/거래유형 조회                                                                                                                                                        |
...
| 운영 자산      | 보험 계약, 차량 기본 정보, 차량 연료 이력, 차량 정비 이력, 차량 운영 요약, 연료/정비 이력의 선택적 수집 거래 연동                                                                                                                          |
...
운영 중에는 기본적으로 하나의 최신 진행월만 열어 두고, 다음 월은 최근 월을 마감한 뒤 엽니다. 업로드 배치가 운영월을 자동으로 만드는 흐름은 운영 시작 전 기초데이터 적재 또는 신규 계좌/카드 등록 직후의 기초 업로드 예외로 제한합니다.
